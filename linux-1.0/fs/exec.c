...
restart_interp:
	...
	i = bprm.inode->i_mode;
	// 检查用户有没有权限执行
	if (IS_NOSUID(bprm.inode) && // 不是超级用户
	...
	old_fs = get_fs();
	set_fs(get_ds());
	retval = read_exec(bprm.inode,0,bprm.buf,128); // 读取可执行文件头128个字节到bprm.buf中
	set_fs(old_fs);
	if (retval < 0)
		goto exec_error2;
	if ((bprm.buf[0] == '#') && (bprm.buf[1] == '!') && (!sh_bang)) { // 如果是shell脚本
		/*
		...
		*cp = '\0';
		...
		for (cp = bprm.buf+2; (*cp == ' ') || (*cp == '\t'); cp++); // 清除头部的空白
		if (!cp || *cp == '\0') {
		 ...
		 */
		retval = open_namei(interp, 0, 0, &bprm.inode, NULL); // 打开解析器程序, 然后重新执行文件
		if (retval)
			goto exec_error1;
		goto restart_interp;
	}
	if (!sh_bang) { // 如果不是shell脚本
		bprm.p = copy_strings(bprm.envc,envp,bprm.page,bprm.p,0);
...
asmlinkage int sys_execve(struct pt_regs regs)
{
	...
	error = getname((char *) regs.ebx, &filename); // 复制用户空间的文件名到内核空间
	if (error)
		return error;
...
beyond_if:
	...
	regs->eip = ex.a_entry;		/* eip, magic happens :-) */ // 设置程序入口
